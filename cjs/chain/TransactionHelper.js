...
const secureRandom = __importStar(require("secure-random"));
...
helper.signed_tr_json = function (tr, private_keys) {
    var tr_buffer = serializer_1.ops.transaction.toBuffer(tr);
    tr = serializer_1.ops.transaction.toObject(tr);
    tr.signatures = (() => {
        var result = [];
        for (var i = 0; 0 < private_keys.length
            ...
            result.push(ecc_1.Signature.signBuffer(tr_buffer, private_key).toHex());
        }
        return result;
    })();
    return tr;
...
helper.template = function (serializer_operation_type_name, debug = { use_default: true, annotate: true }) {
    var so = serializer_1.ops[serializer_operation_type_name];
    if (!so) {
...
exports.TransactionHelper = helper;
