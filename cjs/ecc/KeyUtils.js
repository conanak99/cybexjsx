...
const key = {
    ...
    aes_checksum(password) {
        ...
        var salt = secure_random_1.default.randomBuffer(4).toString("hex");
        var iterations = 0;
        var secret = salt + password;
        // hash for .1 second
        var start_t = Date.now();
        while (Date.now() - start_t < HASH_POWER_MILLS) {
            secret = hash_1.sha256(secret);
            iterations += 1;
        }
        var checksum = hash_1.sha256(secret);
        var checksum_string = [
            iterations,
            salt.toString("hex"),
            checksum.slice(0, 4).toString("hex")
        ].join(",");
        return { aes_private: aes_1.default.fromSeed(secret), checksum: checksum_string };
    },
    /**
    ...
    aes_private(password, key_checksum) {
        ...
        var new_checksum = hash_1.sha256(secret);
        if (!(new_checksum.slice(0, 4).toString("hex") === checksum)) {
            throw new Error("wrong password");
        }
        return aes_1.default.fromSeed(secret);
    },
    /**
    ...
    random32ByteBuffer(entropy = this.browserEntropy()) {
        ...
        hash_array.push(secure_random_1.default.randomBuffer(32));
        return hash_1.sha256(Buffer.concat(hash_array));
    },
    ...
    get_random_key(entropy) {
        return PrivateKey_1.default.fromBuffer(this.random32ByteBuffer(entropy));
    },
    get_brainPrivateKey(brainKey, sequence = 0) {
        ...
        return PrivateKey_1.default.fromBuffer(hash_1.sha256(hash_1.sha512(brainKey + " " + sequence)));
    },
    ...
    browserEntropy() {
        ...
        try {
            ...
            entropyStr = hash_1.sha256(new Date().toString());
        }
    ...
    addresses(pubkey, address_prefix = "CYB") {
        var public_key = PublicKey_1.default.fromPublicKeyString(pubkey, address_prefix);
        // S L O W
        var address_string = [
            address_1.default.fromPublic(public_key, false, 0).toString(address_prefix),
            address_1.default.fromPublic(public_key, true, 0).toString(address_prefix),
            address_1.default.fromPublic(public_key, false, 56).toString(address_prefix),
            address_1.default.fromPublic(public_key, true, 56).toString(address_prefix),
